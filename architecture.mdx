...
sequenceDiagram
    ...
    participant SrcGW as Source Gateway (EVM)
    participant Vald as vald sidecars (all validators)
    participant Chain as axelard (on-chain state)
    participant DstGW as Destination Gateway (EVM)
    ...
    Note over Vald: vald polls EVM RPC, detects ContractCall log

    Vald->>Chain: MsgVoteEvents (cast vote on poll)
    Note over Chain: vote module accumulates votes; quorum reached, poll finalized

    Chain->>Chain: evm EndBlocker: confirmed event to nexus GeneralMessage

    Chain->>Chain: nexus EndBlocker: route message to destination chain queue

    Chain->>Chain: evm EndBlocker: GeneralMessage to ApproveContractCall command batch

    Chain->>Chain: multisig: signing session initiated, snapshot selects participants

    Note over Vald: vald sees SigningStarted event, calls tofnd for threshold signature

    Vald->>Chain: MsgSubmitSignature (each validator)
    Note over Chain: multisig assembles threshold sig; batch status SIGNED

    Note over DstGW: Off-chain relayer reads signed batch from axelard and submits

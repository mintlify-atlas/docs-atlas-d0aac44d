...
sidebarTitle: "Vald"
description: "Learn how vald monitors EVM chains, participates in threshold signing via tofnd, and connects to axelard over RPC. Covers configuration, startup flags, and proxy registration."
---
...
<ParamField path="--from" type="string">
  Name or address of the broadcaster key in the local keyring. This account signs and broadcasts transactions on behalf of the validator. Falls back to the `broadcast.broadcaster-account` field in `vald.toml`.
</ParamField>
...
<ParamField path="broadcast.broadcaster-account" type="string">
  Default keyring key name used as the broadcaster when `--from` is not passed on the command line.
</ParamField>
...
If the tofnd key material needs to be recovered from a mnemonic, pass the recovery file path when starting vald:

```bash
axelard vald-start \
  --tofnd-recovery=/path/to/recovery.json \
  --validator-addr axelarvaloper1... \
  --from broadcaster
```

The Docker entrypoint also supports this through the `RECOVERY_FILE` environment variable.

---
title: "Multisig Module: Threshold Signing for Cross-Chain Ops"
sidebarTitle: "Multisig"
description: "The Multisig module coordinates threshold multi-party key generation and signing sessions, enabling Axelar validators to collectively sign EVM command batches and rotate gateway keys."
---
...
<Note>
Signing sessions have a `SigningTimeout` (default 50 blocks) and a `SigningGracePeriod` (default 3 blocks) after expiry during which late submissions are still accepted.
</Note>
...
<Warning>
The `KeygenTimeout` and `SigningTimeout` were updated from 10 to 50 blocks in v1.3.0 to match 1-second block times. In practice this preserves approximately the same wall-clock duration as before.
</Warning>
